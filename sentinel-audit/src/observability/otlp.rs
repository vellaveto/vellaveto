...
//!
//! Maps Sentinel `SecuritySpan` traces to OpenTelemetry spans with GenAI
...
//! - `gen_ai.system` → `"sentinel"`
//! - `gen_ai.operation.name` → tool name
//! - Custom `sentinel.*` attributes for security-specific data.
//!
//! # Feature Gate
//!
//! Requires the `otlp-exporter` feature flag:
//! ```toml
//! sentinel-audit = { version = "2.2", features = ["otlp-exporter"] }
//! ```
...
/// OTLP exporter configuration (runtime, not the sentinel-config type).
#[derive(Debug, Clone)]
...
///
/// Converts Sentinel SecuritySpan instances to OpenTelemetry spans and
...
///
/// Maps sentinel-specific fields to a combination of GenAI semantic
/// conventions and custom `sentinel.*` attributes.
pub fn span_to_otel_attributes(span: &SecuritySpan) -> Vec<KeyValue> {
    let mut attrs = Vec::with_capacity(16);

    // GenAI semantic conventions
    attrs.push(KeyValue::new("gen_ai.system", "sentinel"));
    ...
    if let Some(ref policy) = span.matched_policy {
        attrs.push(KeyValue::new("sentinel.policy.id", policy.clone()));
    }

    if let Some(ref reason) = span.verdict.reason {
        attrs.push(KeyValue::new("sentinel.verdict.reason", reason.clone()));
    }

    if let Some(ref agent_id) = span.action.agent_id {
        attrs.push(KeyValue::new("sentinel.agent.id", agent_id.clone()));
    }
...
mod tests {
    ...
    fn test_span_to_otel_attributes() {
        ...
        assert!(find("sentinel.tool.name").is_some());
        assert!(find("sentinel.tool.function").is_some());
        assert!(find("sentinel.verdict").is_some());
        assert!(find("sentinel.policy.id").is_some());
    }
    ...
    fn test_genai_semantic_conventions() {
        ...
        let system = attrs
            ...
            .unwrap();
        assert_eq!(system.value.as_str(), "sentinel");
    ...
    fn test_detection_to_event() {
        ...
        let detection_type = attrs
            .iter()
            .find(|kv| kv.key.as_str() == "sentinel.detection.type")
            .unwrap();

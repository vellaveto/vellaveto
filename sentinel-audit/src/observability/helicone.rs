...
impl HeliconeExporter {
    ...
    fn span_to_log(&self, span: &SecuritySpan) -> HeliconeLog {
        ...
        if let Some(policy) = &span.matched_policy {
            properties.insert("sentinel_matched_policy".to_string(), policy.clone());
        }

        if !span.detections.is_empty() {
            properties.insert(
                "sentinel_detection_count".to_string(),
                span.detections.len().to_string(),
            );
            properties.insert(
                "sentinel_max_severity".to_string(),
...
mod tests {
    ...
    fn test_span_to_log() {
        ...
        assert!(log.properties.contains_key("sentinel_tool"));
    }

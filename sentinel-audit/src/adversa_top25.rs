...
//!
//! Maps Sentinel security detections to the Adversa AI MCP Security TOP 25
//! vulnerability ranking. This registry provides a ranked coverage matrix
//! showing which vulnerabilities Sentinel mitigates and how.
//!
...
//!
//! ```ignore
//! use sentinel_audit::adversa_top25::AdversaTop25Registry;
//!
...
use crate::atlas::SentinelDetection;
...
pub struct AdversaVulnerability {
    ...
    /// Sentinel mitigations that address this vulnerability.
    pub sentinel_mitigations: Vec<String>,
}
...
pub struct CoverageMatrixRow {
    ...
    /// Severity level.
    pub severity: AdversaSeverity,
    /// Whether Sentinel covers this vulnerability.
    pub covered: bool,
    /// Sentinel mitigations (empty if not covered).
...
pub struct AdversaCoverageReport {
    ...
    /// Number of vulnerabilities covered by Sentinel.
...
pub struct AdversaTop25Registry {
    ...
    /// Mapping from Sentinel detection to vulnerability ranks.
    detection_mappings: HashMap<SentinelDetection, Vec<u8>>,
}
...
impl AdversaTop25Registry {
    ...
    /// Get vulnerabilities mapped to a Sentinel detection.
    ...
    pub fn coverage_matrix(&self) -> Vec<CoverageMatrixRow> {
        ...
        for v in &self.vulnerabilities {
            if !v.sentinel_mitigations.is_empty() {
        ...
        self.vulnerabilities
            ...
            .map(|v| CoverageMatrixRow {
                ...
                mitigations: v.sentinel_mitigations.clone(),
            })
            .collect()
    }
    ...
    fn map_detection(&mut self, detection: SentinelDetection, ranks: Vec<u8>) {
    ...
    fn populate_vulnerabilities(&mut self) {
        ...
        self.add_vulnerability(AdversaVulnerability {
            ...
            description: "Smuggling authentication tokens through transport manipulation."
                .to_string(),
            severity: AdversaSeverity::High,
            sentinel_mitigations: vec!["Token smuggling detection".to_string()],
        });
        ...
        self.add_vulnerability(AdversaVulnerability {
            ...
            description: "Using glitch tokens to confuse tokenizers and bypass security."
                .to_string(),
            severity: AdversaSeverity::Medium,
            sentinel_mitigations: vec!["Glitch token detection".to_string()],
        });
    }

    /// Populate mappings from Sentinel detections to vulnerability ranks.
    fn populate_detection_mappings(&mut self) {
        // Critical vulnerabilities
        self.map_detection(SentinelDetection::ToolAnnotationChange, vec![1]);
        self.map_detection(SentinelDetection::PromptInjection, vec![2]);
        self.map_detection(SentinelDetection::IndirectInjection, vec![2]);
        self.map_detection(SentinelDetection::ToolSquatting, vec![3, 21]);
        self.map_detection(SentinelDetection::SchemaPoisoning, vec![4]);
        self.map_detection(SentinelDetection::ConfusedDeputy, vec![5]);
        self.map_detection(SentinelDetection::DataLaundering, vec![6]);
        self.map_detection(SentinelDetection::MemoryInjection, vec![6]);
        self.map_detection(SentinelDetection::ShadowAgent, vec![7]);

        // High severity
        self.map_detection(SentinelDetection::PrivilegeEscalation, vec![8]);
        self.map_detection(SentinelDetection::UnauthorizedDelegation, vec![8]);
        self.map_detection(SentinelDetection::SecretsInOutput, vec![9]);
        self.map_detection(SentinelDetection::CovertChannel, vec![9]);
        self.map_detection(SentinelDetection::Steganography, vec![9]);
        self.map_detection(SentinelDetection::UnicodeManipulation, vec![10]);
        self.map_detection(SentinelDetection::PathTraversal, vec![11]);
        self.map_detection(SentinelDetection::DnsRebinding, vec![12]);
        self.map_detection(SentinelDetection::TokenSmuggling, vec![14]);
        self.map_detection(SentinelDetection::GoalDrift, vec![15]);

        // Medium severity
        self.map_detection(SentinelDetection::RateLimitExceeded, vec![16]);
        self.map_detection(SentinelDetection::ContextFlooding, vec![17]);
        self.map_detection(SentinelDetection::CircuitBreakerTriggered, vec![18]);
        self.map_detection(SentinelDetection::CascadingFailure, vec![18]);
        self.map_detection(SentinelDetection::WorkflowBudgetExceeded, vec![18]);
        self.map_detection(SentinelDetection::GlitchToken, vec![25]);

        // Cross-mappings
        self.map_detection(SentinelDetection::DelimiterInjection, vec![2]);
        self.map_detection(SentinelDetection::SecondOrderInjection, vec![2]);
        self.map_detection(SentinelDetection::ExcessiveAgency, vec![8]);
        self.map_detection(SentinelDetection::UnauthorizedToolAccess, vec![8]);
        self.map_detection(SentinelDetection::ToolShadowing, vec![21]);
        self.map_detection(SentinelDetection::SamplingAttack, vec![22]);
    }
}
...
mod tests {
    ...
    fn test_each_vulnerability_has_mitigations() {
        ...
        for vuln in &registry.vulnerabilities {
            assert!(
                !vuln.sentinel_mitigations.is_empty(),
    ...
    fn test_serde_roundtrip_vulnerability() {
        let vuln = AdversaVulnerability {
            ...
            severity: AdversaSeverity::Critical,
            sentinel_mitigations: vec!["m1".to_string()],
        };

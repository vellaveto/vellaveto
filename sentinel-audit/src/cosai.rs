...
//!
//! Maps Sentinel security detections to the 12 CoSAI MCP threat categories
...
//!
//! ```ignore
//! use sentinel_audit::cosai::CosaiRegistry;
//!
...
pub struct CosaiThreat {
    ...
    /// Sentinel capabilities that mitigate this threat.
...
pub struct CategoryCoverage {
    ...
    /// Threats with at least one Sentinel detection mapped.
...
pub struct CosaiCoverageReport {
    ...
    pub total_threats: usize,
    /// Threat IDs with at least one Sentinel detection mapped.
    pub covered_threats: Vec<String>,
    /// Threat IDs without any Sentinel detection mapped.
...
/// Registry of CoSAI threats with mappings to Sentinel detections.
pub struct CosaiRegistry {
    /// All registered threats keyed by ID.
    threats: HashMap<String, CosaiThreat>,
    /// Mapping from Sentinel detection to CoSAI threat IDs.
    detection_mappings: HashMap<SentinelDetection, Vec<String>>,
}
...
impl CosaiRegistry {
    ...
    /// Get Sentinel detections that address a specific CoSAI threat.
    pub fn get_detections_for_threat(&self, threat_id: &str) -> Vec<SentinelDetection> {
        self.detection_mappings
            .iter()
    ...
    fn map_detection(&mut self, detection: SentinelDetection, threat_ids: Vec<&str>) {
    ...
    fn populate_detection_mappings(&mut self) {
        ...
        self.map_detection(SentinelDetection::MemoryInjection, vec!["COSAI-06.1"]);
        self.map_detection(SentinelDetection::GoalDrift, vec!["COSAI-06.2"]);
        self.map_detection(SentinelDetection::ContextFlooding, vec!["COSAI-06.3"]);

        // Supply Chain (covered by tool registry trust + ETDI — no direct SentinelDetection variant)
        ...
        // Transport Security
        self.map_detection(SentinelDetection::TokenSmuggling, vec!["COSAI-08.2"]);

        // Denial of Service
        self.map_detection(SentinelDetection::RateLimitExceeded, vec!["COSAI-09.1"]);
        self.map_detection(
...
mod tests {
    ...
    fn test_threats_for_detection() {
        let registry = CosaiRegistry::new();
        let threats = registry.get_threats_for_detection(SentinelDetection::PromptInjection);
    ...
    fn test_detections_for_threat() {
        ...
        assert!(detections.contains(&SentinelDetection::ToolAnnotationChange));
    }

...
async fn log_single_entry_and_retrieve() {
    ...
    logger.log_entry(&action, &Verdict::Allow, json!({"user": "tester"})).await.unwrap();
...
async fn log_deny_verdict() {
    ...
    logger.log_entry(&action, &Verdict::Deny { reason: "Dangerous operation".to_string() }, json!({})).await.unwrap();
...
async fn log_require_approval_verdict() {
    ...
    logger.log_entry(&action, &Verdict::RequireApproval { reason: "Needs human review".to_string() }, json!({})).await.unwrap();
...
async fn multiple_entries_preserve_order() {
    ...
    for i in 0..5 {
        ...
        logger.log_entry(&action, &Verdict::Allow, json!({"index": i})).await.unwrap();
    }
...
async fn concurrent_writes_all_captured() {
    ...
    for i in 0..20 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            lg.log_entry(&action, &Verdict::Allow, json!({})).await.unwrap();
        }));
    }
    ...
    let entries = logger.load_entries().await.unwrap();
    assert_eq!(entries.len(), 20, "All 20 concurrent writes must be captured");
}
...
async fn large_metadata_does_not_corrupt_log() {
    ...
    logger.log_entry(&action, &Verdict::Allow, serde_json::Value::Object(meta)).await.unwrap();
...
async fn unicode_in_action_fields() {
    ...
    logger.log_entry(&action, &Verdict::Allow, json!({"note": "유니코드"})).await.unwrap();
...
async fn load_entries_is_idempotent() {
    ...
    assert_eq!(entries1.len(), entries2.len(), "load_entries must be idempotent");
}

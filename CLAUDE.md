...
> **State:** v4.0.0-dev (Phases 1–25.1/25.2/25.6 + 26 + 27 + 29 + 30 + 33 + 34 + 35 + 37 + 38 + 39 + 40 + 41 + 43 + 44 + 47 + 48 + 49 complete, 224 audit rounds)
> **Version:** 4.0.0-dev
> **License:** AGPL-3.0 dual license (see LICENSING.md)
> **Tests:** 7,697 Rust tests + 385 Python SDK tests + 127 Go SDK tests + 119 TypeScript SDK tests, zero warnings, zero `unwrap()` in library code
> **Fuzz targets:** 24
...
All 24 phases + Phase 25 (sub-phases 25.1/25.2/25.6) + Phase 26 + Phase 27 + Phase 29 + Phase 30 + Phase 33 + Phase 34 + Phase 35 + Phase 37 + Phase 38 + Phase 40 + Phase 41 + Phase 43 + Phase 44 + Phase 47 + Phase 48 + Phase 49 implemented, tested, and hardened through 224 audit rounds. Details in CHANGELOG.md.
...
- Round 216 (52 findings — 27 P2 + 25 P3: per-entry validation added to ABAC PrincipalConstraint/ActionConstraint/ResourceConstraint/AbacEntity string vectors (MAX_PATTERN_LEN/MAX_CLAIM_KEY_LEN/MAX_TAG_LEN bounds + has_dangerous_chars), PermissionUsage/DiscoveryResult/EvidencePackStatus/TaskResumeResult/TaskIntegrityResult/SecureTaskStats/AttestationVerificationResult/NhiBehavioralDeviation validate() added, CapabilityVerification deny_unknown_fields + issuer_public_key Debug redaction, deny_unknown_fields on 6 trace/explanation structs in core.rs + 3 gap_analysis structs, DidPlcGenesisOperation per-entry has_dangerous_chars, NhiDelegationLink permissions/scope_constraints + NhiBehavioralBaseline tool_call_patterns/typical_source_ips per-entry validation, SyslogConfig::validate() with RFC 5424 SD-ID grammar, WebhookExporter/DatadogExporter config.validate() wiring, DORA/NIS2 fail-closed 0.0% on empty assessments (was 100.0%) + DoraReport/Nis2Report/GapAnalysisReport validate(), has_dangerous_chars on config injection extra/disabled patterns + shadow agent fingerprints + sampling allowed_models + PII patterns + DLP extra_patterns + data_flow zero-value rejection, AuditConfig/RateLimitConfig/MultimodalPolicyConfig validate() + zero-RPS rejection, TlsConfig/SpiffeConfig/OpaConfig path+value has_dangerous_chars, CrossAgentConfig trusted_agents per-entry validation, operator crate: DNS label validation in validate_k8s_name + deny_unknown_fields on 8 spec structs + ResourceRequirements/TenantQuotasSpec validate() + per-element rule validation + delete-then-add→direct upsert + Condition::new() truncation + client redirect disabled)
- Round 222 (4 findings — 1 P1 + 3 P2: FIND-R222-004 SessionState.pending_tool_calls pub→pub(crate) preventing bypass of bounded insertion, FIND-R222-005 SessionState.action_history pub→pub(crate) preventing bypass of bounded insertion, FIND-R222-006 WitnessStore::restore() enforces capacity cap preventing unbounded growth on persistent prover failure, FIND-R222-008 HTTP proxy missing injection scanning on task request parameters and extension method parameters — added full scanning parity with PassThrough and gRPC)
- Round 224 (3 findings — 1 P1 + 2 P2: FIND-R224-001 approval expiry persist failure left in-memory Expired but on-disk Pending — rollback in-memory state on persist failure in both approve()/deny() expiry paths, IMP-R224-001 DnsServiceDiscovery refresh_interval zero/sub-second busy-loop DoS — added MIN_REFRESH_INTERVAL=1s/MAX_REFRESH_INTERVAL=86400s bounds, IMP-R224-006 StaticServiceDiscovery accepted endpoints with control chars — added per-endpoint validate() at construction time)

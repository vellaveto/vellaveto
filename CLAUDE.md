...
| **SDKs** | |
| Python SDK: client, langchain, langgraph, composio, redaction (165 tests) | `sdk/python/` |
| Composio integration: guard, modifiers, extractor, scanner (35 tests) | `sdk/python/vellaveto/composio/` |
...
- **SDKs:** Python (sync+async, LangChain/LangGraph/Composio, 165 tests), TypeScript (fetch-based, 15 tests), Go (stdlib-only, 28 tests)
- **Composio Integration:** `ComposioGuard` with `before_execute`/`after_execute` modifier factories for universal Composio provider support (OpenAI, LangChain, CrewAI, AutoGen, Google ADK), client-side response scanning (DLP + injection), `CallChainTracker` (bounded FIFO), slug normalization, target extraction, standalone `execute()` wrapper, 35 tests

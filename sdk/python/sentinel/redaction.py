"""
Client-side parameter redaction for Sentinel SDK.

Strips or masks sensitive parameter values before they are sent to the
Sentinel server for policy evaluation. This prevents secrets from
transiting the network even when the Sentinel server is trusted.
...
class ParameterRedactor:
    """
    Redacts sensitive parameter values before they are sent to Sentinel.

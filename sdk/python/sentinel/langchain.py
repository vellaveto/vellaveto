...
Provides callback handlers and guards for integrating Sentinel policy
...
class SentinelCallbackHandler(BaseCallbackHandler):
    """
    LangChain callback handler for Sentinel policy enforcement.

    This handler intercepts tool calls and evaluates them against Sentinel
    ...
    def __init__(
        ...
        if not HAS_LANGCHAIN:
            raise ImportError(
                "LangChain is required for SentinelCallbackHandler. "
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        Evaluates the tool call against Sentinel policies and raises
        ...
        try:
            ...
            logger.error(f"Sentinel evaluation failed for {tool_name}: {e}")
            if self.raise_on_deny:
...
class SentinelToolGuard:
    """
    Decorator for guarding individual tools with Sentinel policies.
...
def create_guarded_toolkit(
    ...
    """
    Wrap all tools in a LangChain toolkit with Sentinel guards.
    ...
    Returns:
        List of tools with Sentinel guards applied
    """
    if not HAS_LANGCHAIN:
        raise ImportError("LangChain is required. Install with: pip install langchain")

    guard = SentinelToolGuard(client, session_id=session_id, agent_id=agent_id)
    guarded_tools = []

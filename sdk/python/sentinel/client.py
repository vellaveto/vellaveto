"""
Sentinel API client for Python.

Provides synchronous and asynchronous HTTP client for the Sentinel API.
"""
...
from sentinel.types import Action, EvaluationResult, EvaluationContext, Verdict
...
class PolicyDenied(SentinelError):
...
class ConnectionError(SentinelError):
    """Raised when unable to connect to Sentinel server."""
    pass
...
class SentinelClient:
    ...
    def _request(
        ...
        """Make an HTTP request to the Sentinel API."""
        ...
        try:
            ...
            if "Connection" in str(type(e).__name__):
                raise ConnectionError(f"Failed to connect to Sentinel at {url}: {e}")
            raise SentinelError(f"Request failed: {e}")
    ...
    def evaluate(
        ...
        """
        Evaluate a tool call against Sentinel policies.
...
class AsyncSentinelClient:
    ...
    def __init__(
        ...
        if not HAS_HTTPX:
            raise ImportError(
                "AsyncSentinelClient requires 'httpx' package. "
                "Install with: pip install httpx"
            )
    ...
    async def _request(
        ...
        if not self._client:
            raise SentinelError("Client not initialized. Use 'async with' context.")

...
impl EvaluateResponse {
    /// Check if the verdict is Deny (supports multiple gateway response formats).
    fn is_deny(&self) -> bool {
        // Vellaveto format: {"verdict": {"Deny": {"reason": "..."}}}
        if self.body.get("verdict").and_then(|v| v.get("Deny")).is_some() {
            return true;
        }
        // Simple format: {"verdict": "Deny"}
        if self.body.get("verdict").and_then(|v| v.as_str()) == Some("Deny") {
            return true;
        }
        // Simple format: {"verdict": "deny"}
        if let Some(v) = self.body.get("verdict").and_then(|v| v.as_str()) {
    ...
    /// Check if the verdict is Allow.
    fn is_allow(&self) -> bool {
        if let Some(v) = self.body.get("verdict").and_then(|v| v.as_str()) {
            return v.eq_ignore_ascii_case("allow");
        }
        if self.body.get("verdict") == Some(&serde_json::json!("Allow")) {
            return true;
        }
        false
    }

    /// Check if injection was detected in the response.
    fn has_injection_finding(&self) -> bool {
        // Check inspection.injection_detected
        if let Some(insp) = self.body.get("inspection") {
            if insp.get("injection_detected") == Some(&serde_json::json!(true)) {
                return true;
            }
            // Check injection_patterns array
            if let Some(patterns) = insp.get("injection_patterns").and_then(|p| p.as_array()) {
        ...
        if let Some(flags) = self.body.get("flags").and_then(|f| f.as_array()) {
            if flags.iter().any(|f| {
                f.as_str()
                    .is_some_and(|s| s.contains("injection"))
            }) {
                return true;
            }
        }
    ...
    fn has_dlp_finding(&self) -> bool {
        if let Some(insp) = self.body.get("inspection") {
            if let Some(dlp) = insp.get("dlp_findings").and_then(|d| d.as_array()) {
                if !dlp.is_empty() {
                    return true;
                }
            }

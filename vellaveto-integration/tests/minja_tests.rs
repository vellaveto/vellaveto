...
use vellaveto_config::{MemorySecurityConfig, NamespaceConfig};
use vellaveto_mcp::memory_security::MemorySecurityManager;
use vellaveto_types::{NamespaceAccessType, QuarantineDetection, TaintLabel};
...
fn test_namespace_sharing_requires_approval() {
    ...
    runtime().block_on(async {
        ...
        assert!(matches!(
            decision_before,
            vellaveto_types::MemoryAccessDecision::Deny { .. }
        ));
        ...
        assert!(matches!(
            decision_after,
            vellaveto_types::MemoryAccessDecision::Allow
        ));
    });
}
...
fn test_provenance_chain_tracking() {
    ...
    runtime().block_on(async {
        ...
        assert_eq!(
            ...
            vellaveto_types::ProvenanceEventType::ToolResponse
        );
...
fn test_namespace_capacity_limit() {
    ...
    runtime().block_on(async {
        ...
        match result {
            Err(vellaveto_mcp::memory_security::MemorySecurityError::CapacityExceeded(_)) => {}
...
fn test_duplicate_namespace_rejected() {
    ...
    runtime().block_on(async {
        ...
        match result {
            Err(vellaveto_mcp::memory_security::MemorySecurityError::AlreadyExists(_)) => {}

...
use sentinel_http_proxy::proxy::origin;
...
fn bench_call_chain_parsing(c: &mut Criterion) {
    ...
    let single_entry = serde_json::to_string(&vec![sentinel_types::CallChainEntry {
    ...
    let five_entries: Vec<sentinel_types::CallChainEntry> = (0..5)
...
fn bench_privilege_escalation(c: &mut Criterion) {
    ...
    // 1-hop call chain
    let chain_1 = vec![sentinel_types::CallChainEntry {
    ...
    let chain_5: Vec<sentinel_types::CallChainEntry> = (0..5)
        .map(|i| sentinel_types::CallChainEntry {
            agent_id: format!("agent-{i}"),
            tool: "exec_command".to_string(),
            function: "shell".to_string(),
    ...
    // 10-hop call chain
    let chain_10: Vec<sentinel_types::CallChainEntry> = (0..10)
        .map(|i| sentinel_types::CallChainEntry {
            agent_id: format!("agent-{i}"),
            tool: "exec_command".to_string(),
            function: "shell".to_string(),
...
fn bench_audit_context(c: &mut Criterion) {
    ...
    let claims: sentinel_http_proxy::oauth::OAuthClaims =
        serde_json::from_value(serde_json::json!({
    ...
    let chain: Vec<sentinel_types::CallChainEntry> = (0..3)
        .map(|i| sentinel_types::CallChainEntry {
            agent_id: format!("agent-{i}"),
            tool: "tool".to_string(),
            function: "fn".to_string(),

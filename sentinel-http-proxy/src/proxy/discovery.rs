...
use axum::Json;
use sentinel_types::{SdkCapabilities, SdkTier, TransportEndpoint, TransportProtocol};
use serde::Serialize;

use super::{ProxyState, SUPPORTED_PROTOCOL_VERSIONS};

/// Sentinel's declared SDK tier.
///
/// Sentinel provides multi-transport support, advanced threat detection,
/// compliance evidence, and protocol extensions — qualifying as Extended.
pub const SENTINEL_SDK_TIER: SdkTier = SdkTier::Extended;

/// Build SDK capabilities for the current Sentinel instance.
pub fn build_sdk_capabilities() -> SdkCapabilities {
    SdkCapabilities {
        tier: SENTINEL_SDK_TIER,
        capabilities: vec![

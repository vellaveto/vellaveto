...
fn test_classify_tool_call_via_proto() {
    ...
    let json = proto_request_to_json(&req).unwrap();
    let classified = sentinel_mcp::extractor::classify_message(&json);
    assert!(matches!(
        classified,
        sentinel_mcp::extractor::MessageType::ToolCall { .. }
    ));
}
...
fn test_classify_resource_read_via_proto() {
    ...
    assert!(matches!(
        classified,
        sentinel_mcp::extractor::MessageType::ResourceRead { .. }
    ));
}
...
fn test_classify_batch_via_proto() {
    ...
    assert!(matches!(
        classified,
        sentinel_mcp::extractor::MessageType::PassThrough
    ));
}
...
fn test_classify_sampling_request_via_proto() {
    ...
    assert!(matches!(
        classified,
        sentinel_mcp::extractor::MessageType::SamplingRequest { .. }
    ));
}
...
// ═══════════════════════════════════════════════════════════════════════
// GrpcTransportConfig tests (sentinel-config)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn test_grpc_transport_config_defaults() {
    let config = sentinel_config::GrpcTransportConfig::default();
...
fn test_grpc_transport_config_serde() {
    ...
    let config: sentinel_config::GrpcTransportConfig = serde_json::from_str(json).unwrap();
    assert!(config.enabled);

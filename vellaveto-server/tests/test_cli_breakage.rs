...
//! Adversarial CLI tests that try to break the vellaveto binary.
...
fn no_subcommand_exits_nonzero() {
    let output = vellaveto_bin().output().unwrap();
...
fn policies_dangerous_output_is_parseable_config() {
    ...
    // The output should be valid TOML that vellaveto-config can parse
    let config = vellaveto_config::PolicyConfig::from_toml(&stdout)
...
fn help_flag_exits_zero() {
    let output = vellaveto_bin().arg("--help").output().unwrap();
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("vellaveto") || stdout.contains("Vellaveto"));

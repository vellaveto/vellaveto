...
fn start_server(config_content: &str, port: u16) -> (ServerGuard, TempDir) {
    ...
    let child = Command::new(env!("CARGO_BIN_EXE_vellaveto"))
        .args([
            "serve",
            "--port",
            &port.to_string(),
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("failed to start vellaveto server");
...
// Since vellaveto-server's Cargo.toml doesn't include reqwest in dev-dependencies,

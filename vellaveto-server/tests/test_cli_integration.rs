//! CLI integration tests for the vellaveto binary.
...
fn no_subcommand_shows_help_or_error() {
    let output = vellaveto_bin().output().expect("failed to run vellaveto");
...
fn check_valid_toml_config_succeeds() {
    ...
    let output = vellaveto_bin()
        ...
        .output()
        .expect("failed to run vellaveto check");
...
fn evaluate_allowed_action_returns_allow() {
    ...
    let output = vellaveto_bin()
        ...
        .output()
        .expect("failed to run vellaveto evaluate");
...
fn policies_dangerous_preset_outputs_toml() {
    let output = vellaveto_bin()
        ...
        .output()
        .expect("failed to run vellaveto policies");

...
fn evaluate_with_nonexistent_config_fails() {
    let output = vellaveto_bin()
        .args([
            ...
            "y",
            "--config",
            "/tmp/vellaveto_nonexistent_config_12345.toml",
        ])
        .output()
        .unwrap();
...
"#,
    ...
    assert!(
        output.status.success(),
        "Should work without explicit --params"
    );
}
...
fn check_nonexistent_config_fails() {
    let output = vellaveto_bin()
        .args(["check", "--config", "/tmp/vellaveto_nonexistent_98765.toml"])
        .output()
        .unwrap();
...
fn policies_preset_output_is_valid_toml_config() {
    ...
    let check_output = vellaveto_bin()

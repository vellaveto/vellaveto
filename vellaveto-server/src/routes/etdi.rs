...
pub async fn list_tool_signatures(
    ...
    let Some(ref store) = state.etdi_store else {
        return Err((
            ...
            Json(
                json!({"error": "ETDI (Enhanced Tool Definition Integrity) is not enabled. Set [etdi] enabled = true in your config file and restart the server."}),
            ),
        ));
    };
...
pub async fn verify_attestation_chain(
    ...
    let Some(ref chain_manager) = state.etdi_attestations else {
        return Err((
            StatusCode::SERVICE_UNAVAILABLE,
            Json(
                json!({"error": "ETDI attestation tracking is not enabled. Set [etdi] attestation.enabled = true in your config file."}),
            ),
        ));
    };
...
pub async fn list_version_pins(
    ...
    let Some(ref pin_manager) = state.etdi_version_pins else {
        return Err((
            StatusCode::SERVICE_UNAVAILABLE,
            Json(
                json!({"error": "ETDI version pinning is not enabled. Set [etdi] version_pinning.enabled = true in your config file."}),
            ),
        ));
    };

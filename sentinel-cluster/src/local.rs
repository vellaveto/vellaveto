...
/// rate limiters in `sentinel-server`).
...
impl ClusterBackend for LocalBackend {
    ...
    async fn approval_get(
        &self,
        id: &str,
    ) -> Result<sentinel_approval::PendingApproval, ClusterError> {
    ...
    async fn approval_list_pending(
        &self,
    ) -> Result<Vec<sentinel_approval::PendingApproval>, ClusterError> {
    ...
    async fn rate_limit_check(
        ...
        // rate limiters in sentinel-server. The local backend always returns

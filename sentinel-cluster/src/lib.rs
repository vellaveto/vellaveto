//! Distributed state backend for Sentinel clustering.
//!
...
use sentinel_approval::PendingApproval;
use thiserror::Error;
...
impl From<sentinel_approval::ApprovalError> for ClusterError {
    fn from(err: sentinel_approval::ApprovalError) -> Self {
        match err {
            sentinel_approval::ApprovalError::NotFound(id) => ClusterError::NotFound(id),
            sentinel_approval::ApprovalError::AlreadyResolved(id) => {
                ClusterError::AlreadyResolved(id)
            }
            sentinel_approval::ApprovalError::Expired(id) => ClusterError::Expired(id),
            sentinel_approval::ApprovalError::CapacityExceeded(max) => {
                ClusterError::CapacityExceeded(max)
            }
            sentinel_approval::ApprovalError::Validation(msg) => ClusterError::Validation(msg),
            sentinel_approval::ApprovalError::Io(e) => {
                ClusterError::Backend(format!("IO error: {}", e))
            }
            sentinel_approval::ApprovalError::Serialization(e) => {
...
/// Backend for distributed state sharing across Sentinel instances.
///
...
/// Re-export `ClusterConfig` from `sentinel-config` for convenience.
pub use sentinel_config::ClusterConfig;
...
mod tests {
    ...
    fn test_cluster_config_reexport() {
        // Verify ClusterConfig is re-exported from sentinel-config
        let cfg = ClusterConfig::default();
        assert!(!cfg.enabled);
        assert_eq!(cfg.backend, "local");
        assert_eq!(cfg.redis_pool_size, 8);
        assert_eq!(cfg.key_prefix, "sentinel:");
    }

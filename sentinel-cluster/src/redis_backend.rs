//! Redis cluster backend for distributed Sentinel state.
//!
//! Stores approvals and rate limit state in Redis, enabling multiple Sentinel
...
impl RedisBackend {
    ...
    /// Mirrors the logic in sentinel-approval to ensure compatibility.
...
impl ClusterBackend for RedisBackend {
    async fn approval_create(
        ...
        // Validate identity length (mirrors sentinel-approval store-level check)
        if let Some(ref rb) = requested_by {
            if rb.len() > sentinel_approval::MAX_IDENTITY_LEN {
                return Err(ClusterError::Validation(format!(
                    "requested_by exceeds maximum length of {} bytes",
                    sentinel_approval::MAX_IDENTITY_LEN
                )));
            }
        }
    ...
    async fn approval_approve(&self, id: &str, by: &str) -> Result<PendingApproval, ClusterError> {
        ...
        // Self-approval check: delegate to the same logic that sentinel-approval uses.

...
async fn get_statefulset_status(client: &kube::Client, name: &str, namespace: &str) -> (i32, i32) {
    let api: Api<StatefulSet> = Api::namespaced(client.clone(), namespace);
...
fn generate_config_toml(overrides: &Option<crate::crd::VellavetoConfigOverrides>) -> String {
    let mut lines = vec![

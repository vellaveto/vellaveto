//! Role-Based Access Control (RBAC) for the Sentinel HTTP API server.
//!
...
//! 1. JWT claim: `role` or `sentinel_role`
//! 2. Header: `X-Sentinel-Role` (for development/testing)
...
pub enum RoleSource {
    ...
    Jwt,
    /// From X-Sentinel-Role header (development/testing)
    Header,
...
///
/// Supports both `role` and `sentinel_role` claim names for flexibility
...
pub struct RoleClaims {
    ...
    #[serde(default)]
    pub sentinel_role: Option<String>,
...
impl RoleClaims {
    ...
    /// Priority:
    /// 1. `role` claim (direct)
    /// 2. `sentinel_role` claim (namespaced)
    ...
    pub fn effective_role(&self) -> Option<Role> {
        ...
        // Try sentinel_role claim
        if let Some(ref role_str) = self.sentinel_role {
            if let Some(role) = Role::from_str_loose(role_str) {
                return Some(role);
            }
        }
...
pub struct RbacConfig {
    /// Whether to allow role assignment via X-Sentinel-Role header.
...
/// 2. X-Sentinel-Role header (if allow_header_role is true)
/// 3. Default role
...
mod tests {
    ...
    fn test_role_claims_effective_role() {
        ...
        // role takes precedence over sentinel_role
        let claims = RoleClaims {
            sub: None,
            role: Some("admin".into()),
    ...
    fn test_extract_principal_fallback_to_header() {
        ...
        let mut headers = HeaderMap::new();
        headers.insert(
            HeaderName::from_static("x-sentinel-role"),

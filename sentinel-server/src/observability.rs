...
//! `observability-exporters` feature is enabled in sentinel-audit.

use sentinel_audit::observability::{ObservabilityError, RedactionConfig, SecuritySpan};
use sentinel_config::observability::ObservabilityConfig;
use std::sync::atomic::{AtomicU64, Ordering};
use std::sync::Arc;
use tracing::warn;

#[cfg(feature = "observability-exporters")]
use sentinel_audit::observability::{ObservabilityExporter, SamplingConfig, SpanSampler};
#[cfg(feature = "observability-exporters")]
use tokio::sync::mpsc;
#[cfg(feature = "observability-exporters")]
    ...
    impl ObservabilityManager {
        ...
        pub fn new(config: &ObservabilityConfig) -> Result<Option<Self>, ObservabilityError> {
            ...
            if config.arize.enabled {
                ...
                let exporter =
                    sentinel_audit::observability::arize::ArizeExporter::new(arize_config)?;
                exporter_info.push(ExporterInfo {
            ...
            if config.webhook.enabled {
                ...
                let exporter =
                    sentinel_audit::observability::webhook::WebhookExporter::new(webhook_config)?;
                exporter_info.push(ExporterInfo {

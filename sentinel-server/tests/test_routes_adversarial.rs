...
async fn evaluate_allowed_action_returns_allow() {
    ...
    let body = axum::body::to_bytes(resp.into_body(), 1024 * 1024).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert!(json.to_string().contains("Allow"),
...
async fn evaluate_denied_action_returns_deny() {
    ...
    let body = axum::body::to_bytes(resp.into_body(), 1024 * 1024).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert!(json.to_string().contains("Deny"),
...
async fn evaluate_logs_to_audit() {
    ...
    assert!(!entries.is_empty(),
        "Evaluate endpoint should log to audit");
...
async fn evaluate_missing_fields_returns_error() {
    ...
    let resp = app.oneshot(req).await.unwrap();
    assert!(resp.status().is_client_error(),
        "Missing required fields should return client error, got {}", resp.status());
}
...
async fn remove_policy_by_simple_id() {
    ...
    let resp = app.oneshot(req).await.unwrap();
    assert!(resp.status().is_success(),
        "Remove policy should succeed, got {}", resp.status());
...
async fn audit_entries_empty_returns_empty_array() {
    let (state, _tmp) = make_state();
    let app = routes::build_router(state);
    let req = Request::get("/api/audit/entries").body(Body::empty()).unwrap();
    let resp = app.oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = axum::body::to_bytes(resp.into_body(), 1024 * 1024).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
...
async fn audit_report_empty_returns_zero_counts() {
    let (state, _tmp) = make_state();
    let app = routes::build_router(state);
    let req = Request::get("/api/audit/report").body(Body::empty()).unwrap();
    let resp = app.oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = axum::body::to_bytes(resp.into_body(), 1024 * 1024).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
...
async fn reload_with_nonexistent_config_returns_error() {
    ...
    assert!(resp.status().is_server_error() || resp.status().is_client_error(),
        "Reload with nonexistent config should fail, got {}", resp.status());
}
...
"#).unwrap();
    ...
    let resp = app.oneshot(req).await.unwrap();
    assert!(resp.status().is_success(),
        "Reload with valid config should succeed, got {}", resp.status());
...
async fn unknown_route_returns_404() {
    let (state, _tmp) = make_state();
    let app = routes::build_router(state);
    let req = Request::get("/api/nonexistent").body(Body::empty()).unwrap();
    let resp = app.oneshot(req).await.unwrap();
...
async fn get_on_evaluate_endpoint_returns_405() {
    ...
    assert!(resp.status() == StatusCode::METHOD_NOT_ALLOWED || resp.status() == StatusCode::NOT_FOUND,
        "Wrong method should not be 200, got {}", resp.status());
}
...
fn test_bridge(policies: Vec<sentinel_types::Policy>) -> ProxyBridge {
    let dir = std::env::temp_dir().join("sentinel-proxy-test");
    let _ = std::fs::create_dir_all(&dir);
    let audit = Arc::new(sentinel_audit::AuditLogger::new(dir.join("test-audit.log")));
    // Use compiled policies so context-aware evaluation works (R13-LEG-7).
    let engine = sentinel_engine::PolicyEngine::with_policies(false, &policies).unwrap();
    ProxyBridge::new(engine, policies, audit)
}

#[test]
fn test_evaluate_allowed_tool_call() {
    let policies = vec![sentinel_types::Policy {
        id: "*".to_string(),
...
fn test_bridge_traced(policies: Vec<sentinel_types::Policy>) -> ProxyBridge {
    let dir = std::env::temp_dir().join("sentinel-proxy-test-traced");
    let _ = std::fs::create_dir_all(&dir);
    let audit = Arc::new(sentinel_audit::AuditLogger::new(
...
async fn test_extract_tool_annotations_basic() {
    ...
    let _ = std::fs::create_dir_all(&dir);
    let audit = sentinel_audit::AuditLogger::new(dir.join("test-ann.log"));
...
fn test_injection_blocking_builder_default_false() {
    ...
    let audit = Arc::new(sentinel_audit::AuditLogger::new(std::path::PathBuf::from(
        "/dev/null",
    )));
...
async fn test_flagged_tools_persist_to_file() {
    ...
    let bridge = ProxyBridge::new(sentinel_engine::PolicyEngine::new(false), vec![], audit)
...
async fn test_flagged_tools_blocked_after_reload() {
    ...
    let bridge = ProxyBridge::new(
        sentinel_engine::PolicyEngine::new(false),
        vec![],
        audit.clone(),
    )
    .with_flagged_tools_path(flagged_path.clone());

...
impl TokenSecurityAnalyzer {
    ...
    pub fn check_context_budget(
        ...
        let mut contexts = match self.session_contexts.write() {
            Ok(g) => g,
            Err(_) => {
                tracing::error!(target: "vellaveto::security", "RwLock poisoned in TokenSecurityAnalyzer::check_context_budget");
                return Err(ContextFloodingAlert {
    ...
    pub fn set_session_budget(&self, session_id: &str, budget: usize) {
        let mut contexts = match self.session_contexts.write() {
            Ok(g) => g,
            Err(_) => {
                tracing::error!(target: "vellaveto::security", "RwLock poisoned in TokenSecurityAnalyzer::set_session_budget");
                return;
            }
        };
    ...
    pub fn reset_session(&self, session_id: &str) {
        let mut contexts = match self.session_contexts.write() {
            Ok(g) => g,
            Err(_) => {
                tracing::error!(target: "vellaveto::security", "RwLock poisoned in TokenSecurityAnalyzer::reset_session");
                return;
            }
        };
    ...
    pub fn cleanup_expired_sessions(&self, max_age: Duration) {
        let mut contexts = match self.session_contexts.write() {
            Ok(g) => g,
            Err(_) => {
                tracing::error!(target: "vellaveto::security", "RwLock poisoned in TokenSecurityAnalyzer::cleanup_expired_sessions");
                return;
            }
        };
        let now = Instant::now();
    ...
    pub fn get_session_stats(&self, session_id: &str) -> Option<(usize, usize, f32)> {
        let contexts = match self.session_contexts.read() {
            Ok(g) => g,
            Err(_) => {
                tracing::error!(target: "vellaveto::security", "RwLock poisoned in TokenSecurityAnalyzer::get_session_stats");
                return None;
            }
        };

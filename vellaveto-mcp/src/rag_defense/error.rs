...
mod tests {
    ...
    fn test_is_security_violation_exhaustive() {
        ...
        assert!(!RagDefenseError::ContextBudgetExceeded {
            tokens: 100,
            budget: 50
        }
        .is_security_violation());
        assert!(!RagDefenseError::DocumentTooOld {
            age_hours: 100,
            max_hours: 24
        }
        .is_security_violation());
        ...
        assert!(!RagDefenseError::LowDiversity { similarity: 0.99 }.is_security_violation());
        assert!(!RagDefenseError::InsufficientBaseline {
            samples: 1,
            required: 10
        }
        .is_security_violation());
        assert!(!RagDefenseError::InvalidEmbeddingDimension {
            expected: 768,
            actual: 512
        }
        .is_security_violation());
    ...
    fn test_is_limit_exceeded_exhaustive() {
        ...
        .is_limit_exceeded());
        assert!(RagDefenseError::SessionDocumentLimit { count: 10, max: 5 }.is_limit_exceeded());
        assert!(RagDefenseError::AgentEmbeddingLimit {
            count: 200,
            max: 100
        }
        .is_limit_exceeded());
    ...
    fn test_error_display_all_variants() {
        ...
        .contains("abc"));
        assert!(
            RagDefenseError::RetrievalLimitExceeded { count: 30, max: 20 }
                .to_string()
                .contains("30 > 20")
        );
        assert!(RagDefenseError::ContextBudgetExceeded {
        ...
        .to_string()
        .contains("72h > 24h"));
        assert!(RagDefenseError::SessionDocumentLimit { count: 15, max: 10 }
            .to_string()
            .contains("15 > 10"));
        assert!(RagDefenseError::SensitiveDataDetected {

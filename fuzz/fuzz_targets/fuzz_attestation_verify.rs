...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(attestation) =
        serde_json::from_slice::<sentinel_types::AccountabilityAttestation>(data)
    {
        let now = chrono::Utc::now();
        let _ = sentinel_mcp::accountability::verify_attestation(&attestation, None, &now);

        // Also try with a random expected key
        let _ = sentinel_mcp::accountability::verify_attestation(
            &attestation,

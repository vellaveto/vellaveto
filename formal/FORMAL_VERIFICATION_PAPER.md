...
In the implementation, the verified kernel consumes an intermediate record
actually named `ResolvedMatch` in
`vellaveto-engine/src/verified_core.rs`. The wrapper resolves tool matching,
override checks, context outcomes, and conditional-control flags into the
fields that matter for verdict computation, and the pure kernel then factors
evaluation into `compute_single_verdict(rm)` and
`compute_verdict(resolved)`.

The code-level semantics are precise. `compute_single_verdict` returns
`Continue` for an unmatched entry; otherwise it checks rule overrides and
context denials first, then matched deny policies, then conditional policies
(`RequireApproval`, fired-condition verdicts, skip-on-no-match, or implicit
allow), and finally matched allow policies. `compute_verdict` scans the
resulting `ResolvedMatch` sequence in priority-descending, deny-first order and
returns the first decided outcome, defaulting to `Deny` if every element
continues. Production adds a tertiary policy-ID tiebreaker for deterministic
ordering, but that field is outside `ResolvedMatch` because it does not change
the safety properties proved for the kernel.

The repository also checks the refinement at this boundary: Kani harnesses
K46-K48 verify that the inline evaluator in `apply_compiled_policy_ctx`
produces the same single-policy result as constructing the corresponding
`ResolvedMatch` and calling `compute_single_verdict`.

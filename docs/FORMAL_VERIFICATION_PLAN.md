...
VellaVeto has 545+ verification instances across 7 tools (TLA+, Lean 4, Coq, Alloy, Kani, Verus, and the MCPSEC benchmark) — more formal verification than any MCP security tool and most security middleware. But the verification has a structural weakness: the TLA+, Lean, and Coq proofs operate on abstract mathematical models, while the production code is 39K lines of Rust with String operations, HashMaps, glob matching, serde deserialization, and Unicode normalization. The models prove properties about idealized policy evaluation. The Rust code does the actual policy evaluation. The correspondence between them is informal — tested by 10,550+ tests and 24 fuzz targets, but not proven.

...
use sentinel_mcp::inspection::scan_response_for_secrets;
...
fn test_rfc8707_resource_mismatch_error_accessible() {
    ...
    // through sentinel-http-proxy's oauth module. Since we can't easily
    // run the full OAuth flow without a JWKS server, we verify the
    // config plumbing: OAuthConfig accepts expected_resource.
    //
    // Full validation is covered by sentinel-http-proxy unit tests.

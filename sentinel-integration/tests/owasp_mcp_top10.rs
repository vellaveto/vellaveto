...
// Sentinel coverage: AuditLogger redacts sensitive keys and value
//       prefixes by default.
// ═══════════════════════════════════════════════════════════════
...
// Sentinel coverage: Policy engine with priority-based evaluation,
...
// Sentinel coverage: Priority-based deny-override ensures deny
...
//       tool parameters.
// Sentinel coverage: Parameter constraints with regex, glob,
...
// Sentinel coverage: EXCELLENT — SHA-256 hash chain with
...
// Sentinel coverage: All verdicts (Allow, Deny, RequireApproval)
...
// Sentinel coverage: MAX_LINE_LENGTH (1MB) for MCP framing,
...
fn test_owasp_mcp10_normal_sized_message_accepted() {
    let rt = runtime();
    rt.block_on(async {
        use sentinel_mcp::framing::read_message;
        use std::io::Cursor;
        use tokio::io::BufReader;
